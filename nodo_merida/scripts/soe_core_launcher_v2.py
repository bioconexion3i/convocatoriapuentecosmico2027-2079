# soe_core_launcher_v2.py - Núcleo del SOE con VERSION2 (sin warnings)
...
def publicar_telemetria(client, datos):
    payload = {"timestamp": int(time.time()), "datos": datos}
...
if __name__ == "__main__":
    ...
    try:
        ...
        try:
            ...
            # Usar VERSION2 para eliminar el DeprecationWarning [3][4]
            client = mqtt.Client(callback_api_version=CallbackAPIVersion.VERSION2, client_id="NodoFaro_Merida")
            print("🔧 paho-mqtt >= 2.1 detectado, usando CallbackAPIVersion.VERSION2")
        except (ImportError, AttributeError):
            # Fallback para versiones antiguas
            client = mqtt.Client("NodoFaro_Merida")
            print("🔧 paho-mqtt < 2.1 detectado (sin callback_api_version)")
